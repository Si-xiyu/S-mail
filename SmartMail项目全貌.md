...
SmartMail 是一个课程实训项目，目标是做出类 Notion Mail 的 AI 原生邮件工作台。它不是传统邮箱加几个 AI 按钮，而是让摘要、分类、Junk 分流和 Agent 入口成为默认邮件体验的一部分。

## 一句话理解

用户在同一个 SmartMail 服务器内收发邮件；系统自动为新邮件生成摘要和分类；用户在邮件详情中可以呼出当前邮件 Agent，也可以从左侧打开全局 Agent。第一版先稳定跑通核心体验，后续再接入全量邮箱 RAG 问答。
