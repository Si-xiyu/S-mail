...
本文展开根目录 [项目架构与技术选型.md](../../项目架构与技术选型.md) 中的目标 MVP 架构。第一轮原型已经证明三服务骨架可行，下一阶段目标是让基础邮件系统独立可用，并在启用 AI Plugin 后升级为类 Notion Mail 的 AI 邮件工作台。
...
2. 同步、通知与搜索链路：登录后自动拉取、刷新、轻量轮询、新邮件到达提示、关键词搜索和分类过滤。
3. 自动分析链路：邮件投递后生成摘要、分类、Junk 判断、优先级预测、安全风险提示和分析状态。
4. 交互 Agent 链路：右侧邮件详情抽屉内当前邮件问答；左侧全局 Agent 入口保留 RAG Tool 边界，MVP 可 mock。
...
- `mail_event`：打开、回复、星标、删除、标记 Junk 等行为信号。
...
- 后台系统任务。
- 权限窄：只写摘要、分类、Junk、优先级、风险提示、分析状态。
- 不需要用户确认。
- 可以用规则 fallback。
...
MVP 应提供 AI Plugin 开关和 Provider 配置：

- 关闭 AI Plugin：系统回到 Basic Mail Mode，基础邮件能力不受影响。
- 开启 AI Plugin 且无 API Key：使用规则 fallback 支持 Junk、风险提示、优先级等演示能力。
- 开启 AI Plugin 且配置 DeepSeek API Key：启用大模型摘要、分类和当前邮件 Agent。
...
2. 补 Mail Sync 与 New Mail Notification：登录加载、刷新、轮询、未读数和站内提示。
3. 补搜索：主题、发件人、正文、分类。
4. 补附件选择即上传、发送绑定、下载权限。
5. 补类别和 Junk 视图。
6. 实现行为事件记录和优先级预测规则。
7. 实现 AI Plugin 开关、Provider 配置、规则 fallback。
8. 实现 ai_analysis_task 和后台自动分析。
9. 实现当前邮件 Agent。
10. 做 Global Agent + mock RAG Tool。
11. MVP 后接入真实混合检索。
