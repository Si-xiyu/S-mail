...
public class InternalToolService {
    ...
    public void executeAction(InternalMailActionRequest request) {
        MailboxItem item = requireOwnedItem(request.userId(), request.itemId());
        ...
        switch (action) {
            ...
            case "MOVE" -> {
                String target = request.value() == null ? "" : request.value().trim().toUpperCase(Locale.ROOT);
            ...
            case "SET_PRIORITY" -> {
                String priority = request.value() == null ? "" : request.value().trim().toUpperCase(Locale.ROOT);
                if (!Set.of("LOW", "NORMAL", "HIGH").contains(priority)) {
